...
@section('content')
<h1 class="text-3xl font-semibold text-gray-800 mb-6">Editar Fabricante</h1>

<form action="{{ route('fabricantes.update', $fabricante) }}" method="POST" class="bg-white p-6 rounded shadow-md max-w-lg">
    @csrf
    @method('PUT')
    ...
    <div class="mb-4">
        ...
        <input type="text" name="pais" id="pais"
               value="{{ old('pais', $fabricante->pais) }}"
               class="w-full border border-gray-300 p-2 rounded focus:outline-none focus:ring-2 focus:ring-indigo-500">
        @error('pais')
            <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
        @enderror
